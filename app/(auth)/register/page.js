...
export default function RegisterPage() {
  ...
  const urlRole = searchParams.get('role') === 'teacher' ? 'teacher' : 'student';
  ...
  const generateServiceId = (selectedRole, selectedLine) => {
    const prefix = selectedRole === 'teacher' 
      ? (selectedLine === 'yeneta' ? 'YT' : 'FT') 
      : (selectedLine === 'yeneta' ? 'SY' : 'SF');
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (role === 'teacher') {
        ...
        if (profilePhoto) {
          try {
            const photoRef = ref(storage, `teachers/${uid}/profile.jpg`);
            await uploadBytes(photoRef, profilePhoto);
            profilePhotoUrl = await getDownloadURL(photoRef);
          } catch (storageErr) {
            console.warn("Storage upload failed, using fallback:", storageErr);
            profilePhotoUrl = 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150';
          }
        } else {
          profilePhotoUrl = 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150';
        }
        ...
        if (churchDoc) {
          try {
            const docRef = ref(storage, `teachers/${uid}/church-doc.pdf`);
            await uploadBytes(docRef, churchDoc);
            churchDocUrl = await getDownloadURL(docRef);
          } catch (storageErr) {
            console.warn("Storage upload failed, using fallback:", storageErr);
            churchDocUrl = 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf';
          }
        } else {
          churchDocUrl = 'https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf';
        }
      ...
      if (err.code === 'auth/email-already-in-use') {
        setError(lang === 'am' ? 'ይህ ኢሜል ቀደም ሲል በሌላ ሰው ተመዝግቧል።' : 'This email is already in use.');
      } else {
        ...
        <div className="grid grid-cols-2 gap-2 bg-navy-mid p-1 rounded-lg border border-navy-border">
          <button
            ...
            className={`py-2 text-sm font-semibold rounded-md transition-colors ${
              role === 'student' ? 'bg-gold-primary text-navy-deep' : 'text-text-secondary hover:text-white'
            }`}
          >
            {t('studentRole')}
          </button>
          <button
            ...
            className={`py-2 text-sm font-semibold rounded-md transition-colors ${
              role === 'teacher' ? 'bg-gold-primary text-navy-deep' : 'text-text-secondary hover:text-white'
            }`}
          >
            {t('teacherRole')}
          </button>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('fullNameLabel')}</label>
              <input
                type="text"
                required
                value={fullName}
            ...
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('christianNameLabel')}</label>
              <input
                type="text"
                required
                value={christianName}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{lang === 'am' ? 'ኢሜል' : 'Email Address'}</label>
              <input
                type="email"
                required
                value={email}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('genderLabel')}</label>
              <select
                value={gender}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1 col-span-2">
              <label className="text-xs text-text-secondary font-semibold uppercase block">Service Line</label>
              <div className="flex gap-4">
                <label className="flex items-center space-x-2 text-sm cursor-pointer">
                  <input
                    ...
                    onChange={() => setServiceLine('yeneta')}
          ...
          {role === 'student' && (
            <div className="space-y-4 pt-2 border-t border-navy-border">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{t('ageLabel')}</label>
                  <input
                    type="number"
                    min="4"
                    max="99"
                ...
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{lang === 'am' ? 'የትምህርት ጥቅል' : 'Course package'}</label>
                  <select
                    value={courseId}
                    onChange={(e) => setCourseId(e.target.value)}
                    className="w-full px-3 py-2.5 bg-navy-mid border border-navy-border rounded-md text-white focus:outline-none focus:border-gold-primary text-sm"
                  >
                    <option value="meserete-imnet">{t('package1')} (4,200 ETB)</option>
                    <option value="quanquanna-zema">{t('package2')} (4,200 ETB)</option>
                    <option value="diquna-zegajat">{t('package3')} (4,700 ETB)</option>
                    <option value="all-courses">{t('package4')} (7,100 ETB)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="bg-navy-mid border border-gold-primary/30 p-4 rounded-xl space-y-2">
                <div className="flex justify-between items-center text-sm">
                  <span className="font-semibold text-white">{lang === 'am' ? 'የምዝገባ ክፍያ' : 'Registration Fee'}</span>
                  <span className="font-extrabold text-gold-primary">400 ETB</span>
                </div>
          ...
          {role === 'teacher' && (
            <div className="space-y-4 pt-2 border-t border-navy-border">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{lang === 'am' ? 'የፎቶ ምስል (JPG)' : 'Profile Photo (JPG)'}</label>
                  <input
                    type="file"
                ...
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{t('churchDocLabel')}</label>
                  <input
                    type="file"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{t('bankNameLabel')}</label>
                  <select
                    value={bankProvider}
                ...
                <div className="space-y-1">
                  <label className="text-xs text-text-secondary font-semibold uppercase">{t('bankAccountLabel')}</label>
                  <input
                    type="text"
                    required
                    value={bankAccount}
          ...
          >
            {loading ? (lang === 'am' ? 'በመመዝገብ ላይ...' : 'Registering...') : t('btnRegisterSubmit')}
          </button>

          <div className="text-center pt-2 text-xs">
            <Link href="/login" className="text-gold-primary hover:underline font-semibold">

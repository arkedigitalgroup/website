...
function RegisterPageContent() {
    ...
    // OTP Modal and Verification States
    ...
    // Helper: Formats phone to E.164 Ethiopian format
    const formatEthiopianPhone = (phoneNum) => {
    ...
    // Helper: Send welcome SMS
    const sendWelcomeSMS = async (phoneNo, name, userRole) => {
        const firstName = name.split(" ")[0];
        const message = userRole === "teacher"
            ? `ውድ ${firstName}፣ ወደ አርከ እንኳን ደህና መጡ! መገለጫዎ ለክለሳ ቀርቧል። | Welcome to Arke. Profile under review.`
            : `ውድ ${firstName}፣ ወደ አርከ እንኳን ደህና መጡ! ምዝገባዎ ተጠናቅቋል። | Welcome to Arke. Registration complete.`;
        try {
            await fetch('/api/send-sms', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ phone: phoneNo, message })
            });
        } catch (e) {
            console.warn('SMS failed silently:', e);
        }
    };

    // Helper: Send welcome email via EmailJS
    const sendWelcomeEmail = async (userEmail, name, userRole, serviceIdVal) => {
        if (!userEmail) return;
        try {
            await emailjs.send(
                ...
                process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID,
                {
                    to_name: name,
                    to_email: userEmail,
                    role: userRole === "teacher" ? "መምህር / Teacher" : "ተማሪ / Student",
                    service_line: serviceLine === "yeneta" 
                        ? "የኔታ (መንፈሳዊ)" 
                        : "ፊደል (አካዳሚ)",
                    status: userRole === "teacher" 
                        ? "መገለጫዎ ለክለሳ ቀርቧል — Profile Under Review" 
                        : "ምዝገባ ተጠናቅቋል — Registration Complete",
                    service_id: serviceIdVal,
                }
            );
        } catch (e) {
            console.warn('Email send failed silently:', e);
        }
    };
    ...
    const handleRegister = async (e) => {
        ...
        const e164Phone = formatEthiopianPhone(phone);
        setFormattedPhone(e164Phone);

        // Simple validation for E.164 phone length or shape
        if (!/^\+251[79]\d{8}$/.test(e164Phone)) {
            setError(
                lang === "am"
                    ? "እባክዎ ትክክለኛ የኢትዮጵያ ስልክ ቁጥር ያስገቡ (ለምሳሌ 0911000000)"
                    : "Please enter a valid Ethiopian phone number (e.g. 0911000000)"
            );
            setLoading(false);
            return;
        }
        ...
        try {
            if (role === "teacher") {
                ...
                await completeRegistrationProcess(uid, e164Phone);
            }
        } catch (err) {
    ...
    const handleOtpConfirm = async (otpCode) => {
        ...
        const uid = userCredential.user.uid;
        await completeRegistrationProcess(uid, formattedPhone);
        setIsOtpOpen(false);
    };

    const completeRegistrationProcess = async (uid, verifiedPhone) => {
        try {
            ...
            // Write profile data to /teachers or /students
            if (role === "teacher") {
            ...
            await sendWelcomeSMS(verifiedPhone, fullName, role);
            if (email) {
                await sendWelcomeEmail(email, fullName, role, serviceId);
            }
            ...
            console.error("Firestore / post-registration failure:", err);
            setError(err.message || "Registration failed during saving profile.");
        } finally {
            setLoading(false);
            setUploadStatus("");
        }
    ...
    const handleAuthError = (err) => {
        if (err.code === "auth/email-already-in-use") {
            ...
            setError(
                lang === "am"
                    ? "የአገልግሎት ውቅር በFirebase አልነቃም። እባክዎ Firebase Console ይመልከቱ።"
                    : "Authentication service is disabled in Firebase configuration. Please check Firebase Console."
            );
        } else {
            setError(
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-xs text-text-secondary font-semibold uppercase">
                                    ...
                                    {role === "teacher" && (
                                        <span className="text-[10px] text-text-secondary/60 font-normal lowercase">
                                            (optional)
                                        </span>
                                    )}
                                </label>

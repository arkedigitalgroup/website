...
export default function AdminMatching() {
  ...
  const handleMatchConfirm = async () => {
    ...
    try {
      ...
      const paymentRef = doc(db, 'payments', `${selectedStudent.id}-${currentMonth}`);
      await setDoc(paymentRef, {
        id: `${selectedStudent.id}-${currentMonth}`,
        studentId: selectedStudent.id,
        teacherId: selectedTeacher.id,
        month: currentMonth,
        coursePrice: selectedStudent.courseId === 'all-courses' ? 7100 : (selectedStudent.courseId === 'diquna-zegajat' ? 4700 : 4200),
        serviceFee: 100,
        totalAmount: selectedStudent.courseId === 'all-courses' ? 7200 : (selectedStudent.courseId === 'diquna-zegajat' ? 4800 : 4300),
        status: 'pending',
        paidAt: null,
        companyRevenue: selectedStudent.courseId === 'all-courses' ? 1165 : (selectedStudent.courseId === 'diquna-zegajat' ? 805 : 730),
        teacherPayout: selectedStudent.courseId === 'all-courses' ? 6035 : (selectedStudent.courseId === 'diquna-zegajat' ? 3995 : 3570)
      });

      setMatchSuccess(lang === 'am' ? 'መምህሩ በተሳካ ሁኔታ ተመድቧል!' : 'Tutor assigned successfully!');
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading matching records...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-white font-ethiopic">
          {lang === 'am' ? 'የተማሪ-መምህር ምደባ ማዕከል' : 'Student-Teacher Matching Center'}
        </h1>
        <p className="text-sm text-text-secondary">
          Click a student on the list or pin to calculate nearby verified tutors and link them.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-navy-surface border border-navy-border rounded-xl p-4 shadow-lg flex flex-col justify-between">
          <h2 className="text-sm font-semibold text-text-secondary uppercase mb-3 flex items-center space-x-2">
            ...
            <span className="text-[10px] text-text-muted">(Red: Selected Student, Blue: Unmatched, Gold: Tutor)</span>
          </h2>
          ...
          {selectedStudent && (
            <div className="bg-navy-surface border border-gold-primary/30 rounded-xl p-6 shadow-md space-y-4 animate-fadeIn">
              <div className="border-b border-navy-border pb-3 flex justify-between items-center">
                <h3 className="font-bold text-white text-base">Match Request Details</h3>
                <button
                  onClick={() => {
                    setSelectedStudent(null);
                    setSelectedTeacher(null);
              ...
              <div className="space-y-2 text-sm">
                <div>
                  <span className="text-text-secondary font-semibold">Student Name:</span>{' '}
                  <span className="font-bold text-white">{selectedStudent.fullName}</span>
                </div>
                <div>
                  <span className="text-text-secondary font-semibold">Selected Course:</span>{' '}
                  <span className="text-gold-primary capitalize font-semibold">{selectedStudent.courseId}</span>
                </div>
                <div>
                  <span className="text-text-secondary font-semibold">Location Pin:</span>{' '}
              ...
              {selectedTeacher ? (
                <div className="bg-navy-mid border border-success/30 p-3 rounded-lg flex justify-between items-center">
                  <div className="text-xs">
                    <span className="text-success font-bold block uppercase tracking-wider text-[10px]">Tutor Selected</span>
                    <span className="font-bold text-white text-sm">{selectedTeacher.fullName}</span>
                    <span className="text-text-secondary block">
                      ...
                      <span className="text-gold-primary font-bold">
                        {calculateDistance(
                          selectedStudent.locationPin?.lat,
                          selectedStudent.locationPin?.lng,
                          selectedTeacher.locationPin?.lat,
                          selectedTeacher.locationPin?.lng
                        ).toFixed(2)}{' '}
                        km
                      </span>
                    </span>
                  ...
                  >
                    {matching ? 'Assigning...' : t('btnSaveMatch')}
                  </button>
                </div>
              ) : (
                <p className="text-xs text-warning font-semibold bg-warning-faint p-3 rounded border border-warning/20">
                  👉 Click on one of the available tutors listed below to complete assignment.
                </p>
              )}
              ...
              <>
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                  {nearbyTeachers.length === 0 ? (
                    <p className="text-xs text-text-muted text-center py-6">No verified tutors registered.</p>
                  ) : (
                    nearbyTeachers.map((tutor) => (
                      <div
                        ...
                        className={`p-3.5 border rounded-lg cursor-pointer transition-all flex items-center justify-between gap-4 ${
                          selectedTeacher?.id === tutor.id
                            ? 'bg-navy-mid border-gold-primary shadow-gold'
                            : 'bg-navy-mid/50 border-navy-border hover:border-gold-primary/50'
                        }`}
                      >
                        <div className="text-xs space-y-1">
                          <div className="font-bold text-white text-sm">{tutor.fullName}</div>
                          <div className="text-text-secondary">Rating: {tutor.rating} ⭐ | ID: {tutor.serviceId}</div>
                        </div>
                        <div className="text-right">
                          <span className="text-xs font-bold text-gold-primary">
                            {tutor.distance.toFixed(2)} km
                          </span>
                          <span className="text-[10px] text-text-muted block">away</span>
                        </div>
                      </div>
                    ))
                  )}
              ...
              <>
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                  {unmatchedStudents.length === 0 ? (
                    <p className="text-xs text-text-muted text-center py-6">All students have been assigned a tutor.</p>
                  ) : (
                    unmatchedStudents.map((student) => (
                      ...
                      >
                        <div className="text-xs space-y-1">
                          ...
                          <div className="text-text-secondary">Course: {student.courseId} | {student.gender}</div>
                        </div>
            ...
            <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-4">
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                {matchedStudents.length === 0 ? (
                  <p className="text-xs text-text-muted text-center py-6">No matched students yet.</p>
                ) : (
                  matchedStudents.map((student) => {
                    const tutor = teachers.find(t => t.id === student.assignedTeacherId);
                    return (
                      <div key={student.id} className="p-3 bg-navy-mid/30 border border-navy-border rounded-lg flex items-center justify-between text-xs">
                        <div>
                          <div className="font-bold text-white">{student.fullName}</div>
                          <div className="text-text-muted">Assigned to: {tutor ? tutor.fullName : 'Verified Tutor'}</div>
                        </div>

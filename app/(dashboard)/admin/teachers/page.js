...
export default function AdminTeachers() {
  ...
  const filteredTeachers = teachers.filter((teacher) => {
    if (filter === 'pending') return !teacher.verified && teacher.bankAccount; // has filled form but not verified
    if (filter === 'verified') return teacher.verified;
    if (filter === 'rejected') return !teacher.verified && teacher.rejectionReason;
    return true;
  });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading teachers data...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-white font-ethiopic">
            {t('navAbout') === 'About Us' ? 'Teachers Verification' : 'የአስተማሪዎች ማረጋገጫ'}
          </h1>
          <p className="text-sm text-text-secondary">
            Verify submitted church recommendation letters and approve tutors to teach.
          </p>
        </div>
          ...
          {['all', 'pending', 'verified', 'rejected'].map((item) => (
            <button
              ...
              className={`px-3 py-1.5 rounded uppercase ${
                filter === item ? 'bg-gold-primary text-navy-deep' : 'text-text-secondary hover:text-white'
              }`}
            >
              {item}
            </button>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-navy-mid border-b border-navy-border text-xs text-text-secondary uppercase font-bold tracking-wider">
                  ...
                  <th className="p-4 sm:p-5">Brand / Gender</th>
                  <th className="p-4 sm:p-5">Phone &amp; Bank</th>
                  <th className="p-4 sm:p-5">Church Letter</th>
              ...
              <tbody className="divide-y divide-navy-border text-sm">
                {filteredTeachers.map((teacher) => (
                  <tr key={teacher.id} className="hover:bg-navy-hover transition-colors">
                    <td className="p-4 sm:p-5 space-y-1">
                      <div className="font-bold text-white flex items-center space-x-2">
                        {teacher.fullName}
                        {teacher.christianName && (
                          <span className="text-xs text-gold-primary font-normal">({teacher.christianName})</span>
                        )}
                      </div>
                      <div className="text-xs text-text-muted">ID: {teacher.serviceId}</div>
                    </td>
                    <td className="p-4 sm:p-5">
                      <div className="text-white capitalize font-semibold">{teacher.serviceLine}</div>
                      <div className="text-xs text-text-secondary capitalize">{teacher.gender}</div>
                    </td>
                    <td className="p-4 sm:p-5 space-y-1">
                      <div className="text-white font-medium">{teacher.phone}</div>
                      <div className="text-[10px] text-text-secondary">
                        {teacher.bankProvider}: {teacher.bankAccount}
                      </div>
                    </td>
                    <td className="p-4 sm:p-5">
                      {teacher.churchDocUrl ? (
                        <button
                          onClick={() => setSelectedDocUrl(teacher.churchDocUrl)}
                        ...
                        <span className="text-text-muted text-xs">No Letter</span>
                      )}
                    </td>
                    <td className="p-4 sm:p-5">
                      <span className="text-xs text-text-secondary font-mono">
                        {teacher.locationPin?.lat?.toFixed(4)}, {teacher.locationPin?.lng?.toFixed(4)}
                      </span>
                    </td>
                    <td className="p-4 sm:p-5">
                      {teacher.verified ? (
                    ...
                    <td className="p-4 sm:p-5 text-right">
                      {updatingId === teacher.id ? (
                        <span className="text-xs text-text-muted animate-pulse">Syncing...</span>
                      ) : (
                        <div className="flex justify-end space-x-2">
                          ...
                          {(!teacher.rejectionReason || teacher.verified) && (
                            <button
                              onClick={() => handleOpenRejectModal(teacher.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-navy-surface border border-navy-border rounded-xl max-w-4xl w-full h-[85vh] flex flex-col shadow-2xl overflow-hidden relative">
            <div className="p-4 border-b border-navy-border flex justify-between items-center bg-navy-mid">
              <h3 className="font-bold text-white font-ethiopic">Church Recommendation Document</h3>
              <button 

...
export default function AdminOverview() {
  ...
  const fetchStatsAndAnnouncements = async () => {
    try {
      ...
      let totalT = 0, verifiedT = 0, pendingT = 0;
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading overview data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-10">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Welcome to Arke Control Panel. Manage tutors verification, student matching, and finances.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-2 relative overflow-hidden">
          ...
          <span className="text-xs font-semibold text-text-secondary uppercase block">{t('statTotalTeachers')}</span>
          <div className="flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-white">{stats.totalTeachers}</span>
            <span className="text-xs text-success font-semibold">({stats.verifiedTeachers} verified)</span>
          </div>
        </div>
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-2 relative overflow-hidden">
          <div className="absolute top-4 right-4 text-3xl opacity-20">👶</div>
          <span className="text-xs font-semibold text-text-secondary uppercase block">{t('statTotalStudents')}</span>
          <div className="flex items-baseline space-x-2">
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-2 relative overflow-hidden">
          <div className="absolute top-4 right-4 text-3xl opacity-20">💼</div>
          <span className="text-xs font-semibold text-text-secondary uppercase block">{t('statMonthlyRevenue')}</span>
          <div className="flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-gold-primary">{stats.monthlyRevenue.toLocaleString()}</span>
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-2 relative overflow-hidden">
          <div className="absolute top-4 right-4 text-3xl opacity-20">⏳</div>
          <span className="text-xs font-semibold text-text-secondary uppercase block">{t('statPendingVerifications')}</span>
          <div className="flex items-baseline space-x-2">
            <span className="text-3xl font-extrabold text-warning">{stats.pendingVerifications}</span>
            <span className="text-xs text-warning font-semibold">requires review</span>
          </div>
        </div>
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-lg space-y-4">
          ...
          <form onSubmit={handlePostAnnouncement} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">Announcement Title</label>
              <input
                type="text"
                required
            ...
            {/* Target Role */}
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('announcementRole')}</label>
              <select
                value={targetRole}
            ...
            {/* Content */}
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('announcementContent')}</label>
              <textarea
                required
                rows="4"
            ...
            >
              {posting ? 'Publishing...' : t('btnPostAnnouncement')}
            </button>
        ...
        <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-lg space-y-4">
          ...
          <div className="space-y-4 overflow-y-auto max-h-96 pr-2">
            {announcementList.length === 0 ? (
              <p className="text-text-muted text-sm text-center py-10">No announcements published yet.</p>
            ) : (
              announcementList.map((ann) => (
                <div key={ann.id} className="bg-navy-mid border border-navy-border rounded-lg p-4 space-y-2">
                  ...
                  <div className="text-[10px] text-text-muted text-right">
                    {ann.createdAt ? new Date(ann.createdAt.seconds * 1000).toLocaleString() : 'Just now'}
                  </div>
                </div>
              ))
            )}

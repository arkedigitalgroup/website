  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading billing statements...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Review invoices, current month status, and transfer directly to Arke corporate bank accounts. We show only unified all-inclusive rates.
        </p>
      </div>
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md space-y-6">
        ...
        {payments.length === 0 ? (
          <p className="text-sm text-text-muted text-center py-6">No billing statements available yet. Invoices are generated monthly upon tutor match.</p>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-navy-mid border-b border-navy-border text-xs text-text-secondary uppercase font-bold tracking-wider">
                  ...
                  <th className="p-4">Total Amount (Inc. Service Fee)</th>
              ...
              <tbody className="divide-y divide-navy-border font-medium">
                {payments.map((invoice) => {
                  const total = invoice.totalAmount || calcMonthlyTotal(invoice.coursePrice || 4200);
                  const isPaid = invoice.status === 'paid';
                  const isOverdue = invoice.status === 'overdue';

                  return (
                    <tr key={invoice.id} className="hover:bg-navy-hover transition-colors">
                      <td className="p-4 text-white font-bold">{invoice.month}</td>
                      <td className="p-4 text-text-secondary capitalize">
                        Course Package: {student?.courseId}
                      </td>
                      <td className="p-4 text-gold-primary font-bold">{total.toLocaleString()} ETB</td>
                      <td className="p-4">
                        {isPaid ? (
                      ...
                      <td className="p-4 text-right">
                        {isPaid ? (
                          <span className="text-xs text-text-muted">
                            Paid at {invoice.paidAt ? new Date(invoice.paidAt.seconds * 1000).toLocaleDateString() : 'Just now'}
                          </span>
                        ) : (
                          <button
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm">
          <div className="bg-navy-mid border border-navy-border p-4 rounded-lg space-y-1">
            <span className="text-[10px] text-text-secondary uppercase font-semibold block">Commercial Bank of Ethiopia (CBE)</span>
            <div className="text-base font-extrabold text-gold-primary font-mono">1000452367123</div>
            <span className="text-[10px] text-text-muted block">Account: Arke Digital Group</span>
          </div>
          <div className="bg-navy-mid border border-navy-border p-4 rounded-lg space-y-1">
            <span className="text-[10px] text-text-secondary uppercase font-semibold block">Bank of Abyssinia (BOA)</span>
            <div className="text-base font-extrabold text-gold-primary font-mono">45210985</div>
            <span className="text-[10px] text-text-muted block">Account: Arke Digital Group</span>
          </div>
          <div className="bg-navy-mid border border-navy-border p-4 rounded-lg space-y-1">
            <span className="text-[10px] text-text-secondary uppercase font-semibold block">Telebirr Merchant Wallet</span>
            <div className="text-base font-extrabold text-gold-primary font-mono">98520</div>
            <span className="text-[10px] text-text-muted block">Merchant ID: ArkeDigital</span>
          </div>
        </div>
      </div>
      ...
      {payingId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-navy-surface border border-navy-border rounded-xl max-w-md w-full p-6 shadow-2xl space-y-4">
            <div>
              <h3 className="text-lg font-bold text-white">Submit Transfer Receipt Reference</h3>
              <p className="text-xs text-text-secondary">
                Input the transaction reference code (e.g. FT26145892) generated by your bank app to unlock account.
              </p>
            </div>
            ...
            <form onSubmit={handlePaySubmit} className="space-y-4">
              ...
              <div className="flex justify-end space-x-2">
                ...
                >
                  {submitting ? 'Verifying...' : 'Verify Transfer'}
                </button>
              </div>
            </form>
          </div>

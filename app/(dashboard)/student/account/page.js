...
export default function StudentAccount() {
    ...
    const handlePhotoUpload = async (file) => {
        ...
        try {
            ...
            await updateDoc(doc(db, "students", user.uid), { profilePhotoUrl: url });
            return url;
        } catch (e) {
            console.error("Upload failed:", e);
            setError(lang === "am" ? "ፎቶው ሊሰቀል አልቻለም። ድጋሚ ሞክር።" : "File upload failed. Please try again.");
            return null;
        } finally {
        ...
        <div className="space-y-8 max-w-3xl">
            ...
            <div className="border-b border-navy-border pb-6">
                ...
                <div className="mt-5 flex items-center gap-5">
                    ...
                    <div className="flex-grow space-y-1.5">
                        {data && (() => {
                            const fields = [data.fullName, data.christianName, data.phone, data.locationPin?.lat, profilePhotoUrl];
                            ...
                            return (
                                <>
                                    <div className="flex justify-between text-xs text-text-secondary">
                                        ...
                                        <span className={pct === 100 ? "text-success font-bold" : "text-warning font-bold"}>{pct}%</span>
                                    </div>
            ...
            <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 space-y-6 shadow-md">
                <div className="flex items-center gap-3 border-b border-navy-border pb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-text-muted">
                            ID: <span className="font-mono text-gold-primary">{data?.serviceId}</span>
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSave} className="space-y-5">
                    ...
                    <Field label={lang === "am" ? "የትምህርት ጥቅል" : "Course Package"}>
                        <div className="relative">
                            <select disabled value={courseId} className={`${SELECT_CLS} opacity-60 cursor-not-allowed pr-8`}>
                                <option value="meserete-imnet">Meserete Imnet (Foundation of Faith)</option>
                                <option value="quanquanna-zema">Quanquanna & Zema (Liturgical Chant)</option>
                                <option value="diquna-zegajat">Diquna & Zegajat (Deacon Training)</option>
                                <option value="begena">Begena (Sacred Harp)</option>
                                <option value="all-courses">All Courses Bundle</option>
                            </select>
                            <span className="absolute right-3 top-3 text-xs opacity-50" title="Locked">🔒</span>
                        </div>
                    </Field>
                    ...
                    <div className="border-t border-navy-border pt-5 space-y-3">
                        <p className="text-xs font-semibold text-text-secondary uppercase tracking-wider">
                            📍 {lang === "am" ? "አካባቢ (ኬክሮስ / ቁመት)" : "Location Coordinates"}
                        </p>
            ...
            <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                        ...
                        { label: lang === "am" ? "ሚና" : "Role",              value: "Student / Parent" },

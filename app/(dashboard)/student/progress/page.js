...
export default function StudentProgress() {
  ...
  useEffect(() => {
    const fetchProgressData = async () => {
      ...
      try {
        ...
        const reportsQuery = query(collection(db, 'attendanceReports'), where('studentId', '==', user.uid));
  ...
  const progressPercent = Math.min(Math.round((reports.length / sessionsGoal) * 100), 100);

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        <div className="w-10 h-10 border-4 border-gold-primary border-t-transparent rounded-full animate-spin" />
        <p className="text-text-secondary text-sm animate-pulse">Loading progress tracker...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Track session calendars, study topics, and moral/behavioral feedback submitted by your assigned teacher.
        </p>
      </div>
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md space-y-4">
        <div className="flex justify-between items-center text-sm font-semibold">
          <span className="text-white">Active Month Syllabus Coverage</span>
          <span className="text-gold-primary font-bold">{progressPercent}% ({reports.length} / {sessionsGoal} Sessions Completed)</span>
        </div>
        ...
        <p className="text-xs text-text-secondary leading-relaxed">
          Syllabus resets every month. A total of 12 lessons represents 100% monthly target.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md space-y-6">
          ...
          {reports.length === 0 ? (
            <p className="text-sm text-text-muted text-center py-12">No sessions logged yet. Your tutor will submit reports after lessons.</p>
          ) : (
            <div className="space-y-6">
              {reports.map((report, index) => (
                <div key={report.id} className="relative pl-6 border-l-2 border-gold-primary/30 last:border-l-0 pb-6 last:pb-0 space-y-4">
                  {/* Timeline Node dot */}
                  ...
                  <div className="bg-navy-mid border border-navy-border rounded-xl p-5 space-y-3 shadow-sm">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-navy-border/40 pb-2">
                      ...
                      <span className="text-xs text-text-secondary font-semibold font-mono">
                        📅 {report.date ? new Date(report.date.seconds * 1000).toLocaleDateString() : 'Today'}
                      </span>
                    </div>

                    <div className="space-y-1">
                      <h4 className="font-bold text-white text-sm">
                        {lang === 'am' ? report.topicAm : report.topic}
                      </h4>
                      <div className="flex items-center space-x-2 text-xs">
                        <span className="text-text-secondary">Engagement:</span>
                        <span className="text-gold-primary">
                          {'⭐'.repeat(report.engagementStars || 5)}
                          {'☆'.repeat(5 - (report.engagementStars || 5))}
                        </span>
                      </div>
                    </div>

                    <div className="space-y-1">
                      <span className="text-[10px] text-text-secondary font-semibold uppercase block">Tutor Comments:</span>
                    ...
                    {report.mediaUrl && (
                      <div className="pt-2">
                        <span className="text-[10px] text-text-secondary font-semibold uppercase block mb-1">Attached Media:</span>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-navy-surface border border-navy-border rounded-xl p-6 shadow-md space-y-4">
            ...
            <p className="text-xs text-text-secondary leading-relaxed">
              Arke Tutors focus heavily on spiritual, cultural, and moral excellence. In addition to academia/syllabus, we rate students based on:
            </p>
            <ul className="space-y-2.5 text-xs text-text-secondary">
              <li className="flex items-start space-x-2">
                <span className="text-gold-primary">🔹</span>
                <span>Spiritual respect and attention (ሥርዓተ-አምልኮ)</span>
              </li>
              <li className="flex items-start space-x-2">
                <span className="text-gold-primary">🔹</span>
                <span>Moral integrity &amp; listening (ሥነ-ምግባር)</span>
              </li>
              <li className="flex items-start space-x-2">
                <span className="text-gold-primary">🔹</span>

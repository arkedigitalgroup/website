    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        const qSnap = await getDocs(
          query(collection(db, 'students'), where('assignedTeacherId', '==', user.uid))
        );
        const list = [];
        qSnap.forEach((doc) => {
    ...
    try {
      ...
      if (mediaFile) {
        ...
        try {
          ...
          console.warn("Storage upload failed, using mock media link:", storageErr);
          // High quality mockup educational thumbnail
          mediaUrl = 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=400';
        }
      }
      ...
      setError(err.message || 'Could not submit report. Please check your network.');
    } finally {
      setSubmitting(false);
      setUploadStatus('');
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading assigned students...</p>
      </div>
    );
  }
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-8 sm:p-12 shadow-lg space-y-6">
        ...
        <div className="space-y-2 border-b border-navy-border pb-4">
          ...
          <p className="text-sm text-text-secondary">
            Fill this form after completing each tutoring session to submit daily attendance, progress notes, and images.
          </p>
        </div>

        {students.length === 0 ? (
          <p className="text-sm text-warning font-semibold text-center py-8">
            ⚠️ You cannot submit reports until you are matched with students by the admin.
          </p>
        ) : (
          <form onSubmit={handleReportSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">Select Student</label>
              <select
                required
                value={studentId}
            ...
            {/* Date Picker */}
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('dateLabel')}</label>
              <input
                type="date"
                required
                value={date}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-xs text-text-secondary font-semibold uppercase">Today's Topic (English)</label>
                <input
                  type="text"
                  required
                  value={topic}
              ...
              <div className="space-y-1">
                <label className="text-xs text-text-secondary font-semibold uppercase">የእለቱ ርዕስ (Amharic)</label>
                <input
                  type="text"
                  required
                  value={topicAm}
            ...
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase block">{t('engagementLabel')}</label>
            ...
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase">{t('moralNotesLabel')}</label>
              <textarea
                required
                rows="4"
                value={moralNotes}
            ...
            {/* Media Upload */}
            <div className="space-y-1">
              <label className="text-xs text-text-secondary font-semibold uppercase block">{t('mediaLabel')}</label>
              <input
                type="file"
                accept="image/*,video/*"
            ...
            >
              {submitting ? 'Submitting Report...' : t('btnSubmitReport')}
            </button>

...
export default function TeacherPayouts() {
  ...
  useEffect(() => {
    const fetchPayouts = async () => {
      ...
      try {
        ...
        const paymentsQuery = query(collection(db, 'payments'), where('teacherId', '==', user.uid));
  ...
  payments.forEach((payment) => {
    ...
    const payout = payment.teacherPayout || calcTeacherPayout(payment.coursePrice || 4200);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 space-y-4">
        ...
        <p className="text-text-secondary text-sm animate-pulse">Loading payouts overview...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Track your monthly net earnings representing 85% of matched course packages. Paid directly to your bank provider.
        </p>
      </div>
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md">
        ...
        {monthlyList.length === 0 ? (
          <p className="text-sm text-text-muted text-center py-6">No payout statements generated yet.</p>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-navy-mid border-b border-navy-border text-xs text-text-secondary uppercase font-bold tracking-wider">
                  ...
                  <th className="p-4">Payout Amount (85% Net)</th>
              ...
              <tbody className="divide-y divide-navy-border font-medium">
                {monthlyList.map((row) => (
                  <tr key={row.month} className="hover:bg-navy-hover transition-colors">
                    ...
                    <td className="p-4 text-white">{row.studentsCount} students</td>
                    <td className="p-4 text-gold-primary font-bold">{row.amount.toLocaleString()} ETB</td>
      ...
      <div className="bg-navy-surface border border-navy-border rounded-xl p-6 sm:p-8 shadow-md">
        ...
        {students.length === 0 ? (
          <p className="text-sm text-text-muted text-center py-6">No active matched students.</p>
        ) : (
          <div className="space-y-3">
            {students.map((student) => {
              ...
              return (
                <div key={student.id} className="bg-navy-mid border border-navy-border rounded-lg p-4 flex items-center justify-between">
                  <div>
                    <h3 className="font-bold text-white text-sm">{student.fullName}</h3>
                    <p className="text-xs text-text-secondary capitalize">Course: {student.courseId}</p>
                  </div>
                  <div className="text-right">
                    <span className="font-extrabold text-gold-primary text-sm">{netPayout.toLocaleString()} ETB</span>
                    <span className="text-[10px] text-text-muted block">85% Share</span>
                  </div>
                </div>
              );
            })}
